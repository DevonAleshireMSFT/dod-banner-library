...
| Banner Type | `dodbl_bannertype` | Choice | Required | Snapshot of the banner/classification value at acknowledgement time. Options should align to `dodbl_BannerType`: `None`, `DoD`, `CUI`, `U`, `CONFIDENTIAL`, `SECRET`, `TOP SECRET`, plus `Unspecified` (`703870007`) when no recognized classification value was present at consent acknowledgement. `Unspecified` is an audit fallback only, not a displayed classification. Use publisher option-value prefix `70387`. Audit enabled. |
